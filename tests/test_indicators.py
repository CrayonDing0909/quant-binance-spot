...
def _make_ohlcv(n: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    close = np.maximum(close, 10)  # 避免负数
...
class TestRSI:
    def test_range(self, close):
        ...
        assert rsi.min() >= 0, "RSI 不应小于 0"
        assert rsi.max() <= 100, "RSI 不应大于 100"
...
class TestStochastic:
    def test_range(self, df):
        ...
        valid = stoch.dropna()
        assert valid["%K"].min() >= 0, "%K 不应小于 0"
        assert valid["%K"].max() <= 100, "%K 不应大于 100"
...
class TestADX:
    def test_range(self, df):
        ...
        valid = adx.dropna()
        assert valid["ADX"].min() >= 0, "ADX 不应小于 0"
