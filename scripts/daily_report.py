...
def get_current_prices(symbols: list[str], interval: str) -> dict[str, float]:
    ...
    for sym in symbols:
        try:
            ...
            logger.warning(f"无法获取 {sym} 价格: {e}")
    return prices
...
def format_report(stats: dict, strategy_name: str) -> str:
    ...
    lines = [
        ...
        "",
        f"{'─' * 30}",
        f"  {emoji} 总收益: <b>{ret:+.2f}%</b>",
        f"  💰 权益: <b>${stats['equity']:,.2f}</b>",
        f"  💵 现金: ${stats['cash']:,.2f}",
        f"  📦 持仓: ${stats['position_value']:,.2f}",
    ]
    ...
    # 交易统计
    lines.append(f"\n{'─' * 30}")
    lines.append("  <b>交易统计:</b>")
    lines.append(f"  总交易: {stats['total_trades']} 笔")
    if stats["total_trades"] > 0:
        lines.append(f"  胜率: {stats['win_rate']:.1f}% ({stats['wins']}W / {stats['losses']}L)")
        lines.append(f"  总 PnL: ${stats['total_pnl']:+,.2f}")
        lines.append(f"  总手续费: ${stats['total_fees']:,.2f}")

    # 今日交易
    if stats["today_trades"]:
        lines.append(f"\n{'─' * 30}")
        lines.append(f"  <b>今日交易 ({len(stats['today_trades'])} 笔):</b>")
        for t in stats["today_trades"][-5:]:  # 最多显示 5 笔
    ...
    if stats["week_trades"]:
        ...
        lines.append(
            f"\n  {week_emoji} 近 7 天: {len(stats['week_trades'])} 笔, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Paper Trading 每日绩效报表")
    parser.add_argument("-c", "--config", default="config/rsi_adx_atr.yaml")
    parser.add_argument("-s", "--strategy", default=None)
    parser.add_argument("--print-only", action="store_true", help="只打印不发送 Telegram")
    ...
    if state is None:
        print(f"❌ Paper Trading 状态文件不存在: {state_path}")
        print(f"   请先运行: PYTHONPATH=src python scripts/run_live.py -c {args.config} --paper --once")
        return
    ...
    if not args.print_only:
        ...
        if ok:
            ...
            print("\n⚠️  Telegram 发送失败（请检查 .env 中的 Token 和 Chat ID）")
