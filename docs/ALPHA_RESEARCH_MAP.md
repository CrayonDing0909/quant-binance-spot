> **Last updated**: 2026-03-29 (WQ Alpha Composite: wq012+028+055, IC=0.008-0.013, TSMOM corr=-0.004. Standalone SR=0.45(BTC)/0.74(ETH) weak, but 10% blend → portfolio SR 3.77→3.90, MDD -3.9%→-3.5%. PBO 2.2%, DSR p=0.0. 15m FAIL. GO as 10% component. 华山论剑 Convergence v2.2: BTC SR=1.34/MDD-21%, ETH SR=0.95/MDD-22%. Validation: PBO 3.3%, DSR p=0.0, 2x cost SR>0.8. But portfolio blend KEEP_BASELINE — TSMOM corr=0.50, adds no SR improvement to prod. Preserved as validated backup. CB Premium standalone: IC=+0.031 but funding rate kills futures alpha. New: `/quant-signals-reference` skill, BTC vs ETH signal divergence research (10 papers), Obsidian vault.)
...
|| 2026-03-28 | **CB Premium EDA (GO → standalone weak)**: Coinbase-Binance spread IC=+0.031(1h), +0.022(zscore_24h). BTC+ETH both positive. TSMOM corr=0.009. But funding rate kills futures alpha: LO SR=0.45 (FR eats 70%). Spot SR=0.53 only. Signal is real but too thin standalone. Kept as convergence component | Alpha Researcher |
|| 2026-03-28 | **华山论剑 Convergence v1→v2→v2.1→v2.2**: 5-pillar composite (OI 4-quad + FR + LSR + CB Premium + Stablecoin). v2 added ADX gate, ATR SL, persist/cooldown (from LSR v3 patterns). v2.1 changed 4h rebalance → BTC SR=1.34 but ETH SR=0.26. v2.2 per-symbol routing: BTC 4h / ETH daily + lower FR/CB/SC weights → ETH SR=0.95. Validation: PBO 3.3%, DSR p=0.0, WFA pass, 2x cost SR>0.8. **But portfolio blend KEEP_BASELINE: TSMOM corr=0.50, no SR improvement to prod (TSMOM SR=3.85 too strong)**. Preserved as validated backup strategy | Alpha Researcher + Quant Dev |
|| 2026-03-28 | **BTC vs ETH Signal Divergence Research**: 10 papers reviewed. Root causes: ETH FR polluted by staking hedge (87.5% at floor, BitMEX Q3 2025), ETH on-chain confounded by DeFi mechanics (1.3% daily turnover vs BTC 0.61%), ETH info processing slower (Guo et al. 2024). Built per-symbol parameter routing as solution. Full writeup: `docs/research/20260328_btc_eth_signal_divergence.md` | Alpha Researcher |
|| 2026-03-28 | **Infrastructure**: Built `/quant-signals-reference` skill (6 signals + anti-overfit methodology), Obsidian vault at `docs/`, installed 24 new skills (Anthropic pdf/xlsx, K-Dense scientific, ARIS research pipeline). External research: 13 papers across 6 signal types → `docs/research/20260328_huashan_external_research.md` | DevOps |
